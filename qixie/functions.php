...
function admin_footer_text($text) {
       $text = '<span id="footer-thankyou"> <a href="https://www.muruoxi.com" target="_blank" style="font-size: 14px;color: red;">甘肃省大学生棋类联盟</a> Theme by:慕若曦，<a class="qq" rel="nofollow" target="_blank" href="tencent://message/?uin=1171867037&Menu=yes" title="QQ咨询" style="font-size: 14px;color: red;">点击</a> 进行商业合作。</span>';
    return $text;
}
...
function cptui_register_my_cpts_product() {
	$labels = array(
		"name" => __( '赛事管理', 'xs' ),
		"singular_name" => __( '赛事列表', 'xs' ),
		'add_new'            => _x( '新建赛事', '添加新内容的链接名称' ),
		'add_new_item'       => __( '新建一个赛事' ),
		'edit_item'          => __( '编辑赛事' ),
		'new_item'           => __( '新赛事' ),
		'all_items'          => __( '所有赛事' ),
		);
...
function add_new_product_columns($book_columns) {   
    ...
    $new_columns['title'] = '赛事名称';   
    $new_columns['images'] = __('缩略图');   
       
    $new_columns['products'] =__('赛事分类');    
...
add_action('admin_head', 'admin_mycss');
 //输出缩略图地址
function post_thumbnail_src($id=""){
	global $post;
	if(!empty($id)){
		$thumbnail_src = wp_get_attachment_image_src(get_post_thumbnail_id($id),'full');
...
function include_template_function( $template_path ) {
    if ( get_post_type() == 'product' ) {
        if ( is_single() ) {
            $template_path = locate_template( array ( 'page-product.php' ) ); 
        }
    }
...
require get_template_directory()."/flamingo/flamingo.php";
function flamingo_lang(){
    load_theme_textdomain('flamingo', get_template_directory() . '/flamingo/languages');
    }
add_action('after_setup_theme', 'flamingo_lang'); 


require get_template_directory()."/contact-form-7/wp-contact-form-7.php";
function wp_contact_form_7_lang(){
    load_theme_textdomain('contact-form-7', get_template_directory() . '/contact-form-7/languages');
    }
add_action('after_setup_theme', 'wp_contact_form_7_lang');
